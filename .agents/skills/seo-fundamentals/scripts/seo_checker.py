...
SKIP_DIRS = {
    ...
    '__tests__', 'spec', 'docs', 'documentation', 'examples', 'vendor', 'storage'
}
...
def is_page_file(file_path: Path) -> bool:
    ...
    # HTML or Blade files are usually pages
    if file_path.suffix.lower() in ['.html', '.htm'] or name.endswith('.blade.php'):
        return True
...
def check_page(file_path: Path) -> dict:
    ...
    # Clean template expressions to prevent arrow operators (-> or =>) from breaking tag matching
    clean_content = re.sub(r'\{\{.*?\}\}', 'placeholder_value', content, flags=re.DOTALL)
    clean_content = re.sub(r'\{!!.*?!!\}', 'placeholder_value', clean_content, flags=re.DOTALL)
    
    # 4. Heading hierarchy - multiple H1s
    h1_matches = re.findall(r'<h1[^>]*>', clean_content, re.I)
    if len(h1_matches) > 1:
        issues.append(f"Multiple H1 tags ({len(h1_matches)})")
    
    # 5. Images without alt
    img_pattern = r'<img[^>]+>'
    imgs = re.findall(img_pattern, clean_content, re.I)
    for img in imgs:
