        ...
        function openNewsModal(newsId) {
            ...
            if (videoUrl !== '') {
                ...
                if (videoUrl.includes('tiktok.com')) {
                    ...
                    const isReel = videoUrl.includes('/reel/') || videoUrl.includes('/share/r/');
                    if (isReel) {
                        modalWrapper.style.maxWidth = '460px';
                    } else {
                        modalWrapper.style.maxWidth = '680px';
                    }
                    const fbPluginSrc = 'https://www.facebook.com/plugins/video.php?href=' + encodeURIComponent(videoUrl) + '&show_text=false';
                    embedHtml = `
                        <div class="w-full bg-slate-950 flex flex-col items-center py-3 px-2">
                            ${isReel 
                                ? `<iframe src="${fbPluginSrc}" class="w-full h-[520px] max-h-[60vh] border-0 rounded-2xl shadow-lg" style="max-width: 340px; width: 100%; height: 520px; margin: 0 auto; display: block;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen="true" loading="lazy"></iframe>`
                                : `<div class="aspect-video w-full"><iframe src="${fbPluginSrc}" class="w-full h-full border-0 rounded-xl shadow-lg" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen="true" loading="lazy"></iframe></div>`
                            }
                            <a href="${encodeURI(videoUrl)}" target="_blank" rel="noopener noreferrer" 
                               class="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1877F2] hover:bg-[#0d65d9] text-white text-xs font-bold shadow-md transition-all hover:scale-105 active:scale-95">
                                ...
                                <span>Otwórz w aplikacji Facebook</span>
                                <span class="material-symbols-outlined text-sm">open_in_new</span>
                            </a>
                        </div>
                    `;
